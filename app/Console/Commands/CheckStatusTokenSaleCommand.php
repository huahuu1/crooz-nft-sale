    ...
    public function validateTransactions()
    {
        ...
        if (! empty($pendingTransactions)) {
            foreach ($pendingTransactions as $key => $transaction) {
                UpdateStatusTokenSaleJob::dispatch($transaction, $company_wallet, $contract_wallet, $key)->onQueue(config('defines.queue.check_status'))->delay(now()->addSeconds(($key + 1) * 10));
            }
        }
    }
}
